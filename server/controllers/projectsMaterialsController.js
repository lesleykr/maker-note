...
router.post('/createmat', validateSession, (req, res) => {
    ...
    .then(([Projects, Materials]) => PM.create({projectId: Projects.id, materialId: req.body.materials.id}))     
...
router.post('/create', validateSession, (req, res) => {
   Promise.all([
       ...
       .then(([Projects, Materials]) => PM.create({projectId: Projects.id, materialId: Materials.id})) 
       .catch(error => res.status(400).send(error));
});

//directly to pm -working
router.post('/projects/:pid/materials/:mid', async (req, res) => {
    try{
    const projects = await Projects.findOne({where: {id: req.params.pid}})
    const materials = await Materials.findOne({where: {id: req.params.mid}})
...
router.put('/projects/:pid/materials/:mid', async (req, res) => {
    try{
    const updateProjects = await Projects.findOne({where: {id: req.params.pid}})
    const materials = await Materials.findOne({where: {id: req.params.mid}})
    const result = await projects.addProjMat(materials)
    res.json(result)
    } catch (err) {
